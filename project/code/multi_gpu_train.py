...
""" A binary to train the CNN model using multiple GPUs with synchronous updates.
...
MAX_STEPS = 50                                  # Number of batches to run
NUM_GPUS = 1
...
def tower_loss(scope, images, labels):
  ...
  _ = recognizer.loss(logits, labels)
  ...
  for l in losses + [total_loss]:
    ...
    loss_name = re.sub('%s_[0-9]*/' % recognizer.TOWER_NAME, '', l.op.name)
...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    num_batches_per_epoch = (input.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / input.TRAIN_EVAL_BATCH_SIZE)
    decay_steps = int(num_batches_per_epoch * recognizer.NUM_EPOCHS_PER_DECAY)

    # Decay the learning rate exponentially based on the number of steps.
    lr = tf.train.exponential_decay(recognizer.INITIAL_LEARNING_RATE,
                                    global_step,
                                    decay_steps,
                                    recognizer.LEARNING_RATE_DECAY_FACTOR,
                                    staircase=True)
    ...
    with tf.variable_scope(tf.get_variable_scope()):
      for i in xrange(NUM_GPUS):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (recognizer.TOWER_NAME, i)) as scope:
    ...
    variable_averages = tf.train.ExponentialMovingAverage(
        recognizer.MOVING_AVERAGE_DECAY, global_step)
    ...
    for step in xrange(MAX_STEPS):
      ...
      if step % 10 == 0:
        num_examples_per_step = input.TRAIN_EVAL_BATCH_SIZE * NUM_GPUS
