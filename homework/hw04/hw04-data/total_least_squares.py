...
##    imsave('output.png',output)
    ...
    coeff = np.matmul( np.linalg.inv( np.matmul(Bp.T, Bp) ), np.matmul(Bp.T, vsp) )
    ...
    imsave('output_OLS.png',output)

    #################################################
    # Solve for the coefficients using total least squares
    ##################################################

    Ay = np.concatenate( (Bp, vsp), axis=1 )
    U, S, V = np.linalg.svd(Ay, full_matrices=0, compute_uv=1)
    V = V.T
    x = V[:, -3:]
    x_modified = np.matmul(x, (-1)*np.linalg.inv(x[-3:, :]))
    coeff = x_modified[:-3, :]
    ...
    imsave('output_TLS.png',output)

    #################################################
    # Scaling total least squares
    ##################################################
    ...
    imsave('output_TLS_scaled.png',output)
